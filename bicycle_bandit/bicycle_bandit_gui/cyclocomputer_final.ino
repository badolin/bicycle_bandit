 ...
 * outPort = 
 * localPort = 
 * char *names[] = {"Radek", "Kasia", "Igor", "Justyna", "Lukasz", "Magda W", "Iza", "Oleh", "Bartek", "Magda Z", "Kuba", "Andzelika"};   /// gdy ktos odpadnie nalezy skasowac
...
const IPAddress outIp(192,168,1,116);  // komp na 19
const IPAddress localIp(192,168,1,216); // 216 <- Rower w kuchni, 217 <- Rower w TV, 218 <- BuckUp
...
if(millis() - previous > 100){
  ...
  redCircle = false;
	...
	else if((currentImpulse == 0 && (noImpulsesTimer - firstTimeNoImpulses) > zeroVelocityInterval) 
			|| (currentImpulse == previousImpulse && (sameImpulsesTimer - firstTimeSameImpulses) > zeroVelocityInterval))
	{
    impulses = 0;
    distance = 0;
